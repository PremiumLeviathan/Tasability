local ConfigTable = _G.TasabilityConfig

local PlaybackInputs = ConfigTable.PlaybackInputs
local PlaybackMouseLocation = ConfigTable.PlaybackMouseLocation
local RoundDigits = ConfigTable.RoundDigits
local ReplayStartTime = ConfigTable.ReplayStartTime
local FrameBacktrackCount = ConfigTable.FrameBacktrackCount
...
-- Color codes for the color frame
local ColorCodes = {
...
-- Tasability update
local Frozen = false
local FreezeFrame = 0 -- Frame to render while frozen
...
local CurrentPlaceIdLabel = Instance.new("TextLabel")
...
do -- Overwriting JSONDecode
	local jsondecode = loadstring(game:HttpGet("https://raw.githubusercontent.com/plusgiant5/Libraries/main/loadstrings/json.lua"))().decode
	local oldcall
	oldcall = hookmetamethod(game,"__namecall",function(self,...)
		if checkcaller() and self == HttpService and getnamecallmethod() == "JSONDecode" then
			return jsondecode(...)
		end
		return oldcall(self,...)
	end)
...
do
	...
	MainFrame.Size = UDim2.fromOffset(700,300)
	...
	CurrentPlaceIdLabel.Text = "PlaceId: "..tostring(PlaceId)
	CurrentPlaceIdLabel.Size = UDim2.fromOffset(80,60)
	CurrentPlaceIdLabel.Position = UDim2.fromOffset(0,240)
	CurrentPlaceIdLabel.TextScaled = true
	CurrentPlaceIdLabel.Parent = MainFrame
	...
	ConsoleFrame.Size = UDim2.fromOffset(400,300)
...
do
	ConsoleMessage = function(...)
		...
		Label.Size = UDim2.fromOffset(400,20)
		Label.TextSize = 14
...
do
	RenderSteppedConnections.UpdateFreezeFrame = function()
		RecordedFramesLabel.Text = FreezeFrame
	end
...
spawn(function() -- Reading but with Stepped to remove the delay on client
	while true do
		if Reading then
...
spawn(function() -- Reading but with RenderStepped to remove the delay on client
	while true do
		if Reading then
	...
	while true do
		if Frozen then
			...
			if FreezeFrame > 0 and FreezeFrame <= #ReplayTable then
				local Frame = ReplayTable[FreezeFrame]
				...
				if type(Frame) == "table" then
					...
					for Index = FreezeFrame-math.max(FrameBacktrackCount,0),FreezeFrame do
						local Frame = ReplayTable[Index]
						if Frame and type(Frame) == "table" then
...
ConsoleMessage("Tasability loaded in",RoundNumber(tick()-ExecutionTick,2),"seconds")